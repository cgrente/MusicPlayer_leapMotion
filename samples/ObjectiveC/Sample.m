...
- (void)run
{
    ...
    hadChanged = NO;
    tableSimple = [NSArray arrayWithObjects:
                             @"Schoolboy",
                             @"Turn",
                             @"test",
                             @"happy",
    ...
    path = [[NSBundle mainBundle] pathForResource:[tableSimple objectAtIndex:i] ofType:@"mp3"];
    ...
    if ([[frame hands] count] != 0) {
        ...
        if ([fingers count] != 0) {
            ...
            for (int j = 0; j < [fingers count]; j++) {
                LeapFinger *finger = [fingers objectAtIndex:j];
